  ...
  return (
    <Modal isOpen={isOpen} onOpenChange={onOpenChange} isDismissable={false} hideCloseButton>
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader className="flex flex-col gap-1">{t('header', { ns: 'result' })}</ModalHeader>
            <ModalBody>
              <Accordion>
                {resultData.map((data, idx) => {
                  return (
                    <AccordionItem key={`accordion-item-${idx}`} aria-label={`question ${idx}`} title={t(data.questions)}>
                      <strong className="text-xl">{data.answer || t('empty', { ns: 'result' })}</strong>
                    </AccordionItem>
                  )
                })}
              </Accordion>
